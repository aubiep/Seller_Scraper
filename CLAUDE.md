...
|---|---|---|
| `snoco_scraper_v2_8_1.py` | Lead enrichment. **King County LIVE** (ArcGIS + Dashboard). **Snohomish LIVE** via the new Public Access portal, with **full detail** (assessed value, market total, last sale date+price, sales history, year built). Writes to propintel.db. **v2.8.1: restored Snohomish rich detail. After the search resolves the parcel, `snoho_fetch_detail()` reads the parcel's Property Account Summary page and pulls each fact from its "DataDisplay" JSON table (`/API/DataDisplay/DataSources/GetData`, keyed by the table's ModuleId off the page; tables ID'd by column name; MUST send `Accept: application/json` or the server 500s trying XML). Beds/baths/finished-sqft are NOT published on this portal, so they stay blank (only remaining gap vs King). v2.8.0: REBUILT Snohomish onto the portal QuickSearch JSON API after the county retired `snoco.org/proptax` (every lookup 404'd); search = `/DesktopModules/QuickSearch/API/Module/GetData?keywords=<house# + street name>&page=1` with DNN headers ModuleId/TabId/RequestVerificationToken; `get_session`/`scrape_property` keep their signatures; results filtered to input street + city-disambiguated.** v2.7.4: KC relaxed fallback for direction-in-name streets. v2.7.3: KC condo master-parcel handling. v2.7.2: address-clean / detect_county / street-filter fixes. | `python snoco_scraper_v2_8_1.py Seller_Leads.txt` |
