...
|---|---|---|
| `snoco_scraper_v2_8_4.py` | Lead enrichment. **King County LIVE** (ArcGIS + Dashboard). **Snohomish LIVE** via the new Public Access portal, with **full detail INCLUDING beds/baths/sqft** (assessed value, market total, last sale date+price, sales history, year built, bedrooms, full+half baths, finished sqft, grade, condition, exterior, roof, foundation, heat, fireplace - more than King exposes). Writes to propintel.db. **v2.8.4: search falls back to the full spelled-out street when the stripped keyword misses (recovers 'Avenue D', 'S Machias', 'Mountain Loop', unit numbers). v2.8.3: 'Most Recent Sale' uses the newest NON-ZERO sale (skips $0 quit-claims) so the equity score is right. v2.8.2: pulls structure detail from each parcel's Building Detail page (linked off the summary's Buildings table via the DataDisplay row hyperlink); `snoho_fetch_detail()` follows it and reads the building's DataDisplay tables. v2.8.1: value/sale/year from the Property Account Summary page DataDisplay tables (`/API/DataDisplay/DataSources/GetData` keyed by table ModuleId off the page; tables ID'd by column name; MUST send `Accept: application/json` or the server 500s trying XML). v2.8.0: REBUILT Snohomish onto the portal QuickSearch JSON API after `snoco.org/proptax` was retired; `get_session`/`scrape_property` keep their signatures; results filtered to input street + city-disambiguated.** v2.7.4: KC direction-in-name street fallback. v2.7.3: KC condo master-parcel handling. v2.7.2: address fixes. | `python snoco_scraper_v2_8_4.py Seller_Leads.txt` |
...
| `propintel_app_v0_3_5.py` | **The dashboard.** Home = **daily command center** (new & unprocessed leads, follow-up due, priority worklist, review queue, status tiles). **Merged /leads** (Contacts+Leads in one repeat-aware page: "Nx" badge, filters, Add-contact form, per-row queue+enroll; /contacts redirects here). Priority worklist (/priority). Lead detail (/property/<id>) with photos + every submission. **v0.3.5: self-service letter templates (/letters/templates) - upload an iAuto design .json, name it, edit each blank's wording with click-to-insert {placeholder} chips; blanks auto-detected, standard envelope reused, no hand-editing of `config/iauto_templates.json`. Backed by `iauto_send.load_config/save_config`.** v0.3.4: row thumbnails fall back to the county assessor photo when Google has no Street View for the address (rural roads); `street_thumb()` takes the assessor file and the home/priority/leads queries carry `p.assessor_photo_file` (pairs with `streetview_backfill_v0_1_0.py`). v0.3.3: per-lead "Re-run assessor lookup" button (POST /lead/<cid>/<pid>/reenrich) + a "Last assessor lookup" panel showing what the assessor returned on a failure. iAuto letter queue (change template per queued row; inline Enroll-in-Homebot). **Bulk mail** (/letters/bulk: city/county/zip/neighborhood/lead-type/verdict/date + radius, arrange by score/distance, top-N, queue many). Homebot hub. **Edit-contact form** (/contact/<id>/edit) to fix name casing / add email-phone. | `python propintel_app_v0_3_5.py` then open http://127.0.0.1:5000 |
...
| `streetview_backfill_v0_1_0.py` | **Street View photo backfill.** Re-fetches the Google Street View photo for any property that has an address but no saved `photo_file` (older rows scraped before the photo step / before billing). Reuses the scraper's exact download + filename. Pre-checks Google's free metadata endpoint so it only saves real imagery (skips the gray "no imagery" placeholder); genuinely-rural rows stay blank and the dashboard then falls back to the assessor photo. Backfilled 43 rows on first run. | `python streetview_backfill_v0_1_0.py --dry-run` then live |
