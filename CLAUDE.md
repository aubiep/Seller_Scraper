...
|---|---|---|
| `snoco_scraper_v2_8_3.py` | Lead enrichment. **King County LIVE** (ArcGIS + Dashboard). **Snohomish LIVE** via the new Public Access portal, with **full detail INCLUDING beds/baths/sqft** (assessed value, market total, last sale date+price, sales history, year built, bedrooms, full+half baths, finished sqft, grade, condition, exterior, roof, foundation, heat, fireplace - more than King exposes). Writes to propintel.db. **v2.8.3: 'Most Recent Sale' uses the newest NON-ZERO sale (skips $0 quit-claims) so the equity score is right. v2.8.2: pulls structure detail from each parcel's Building Detail page (linked off the summary's Buildings table via the DataDisplay row hyperlink); `snoho_fetch_detail()` follows it and reads the building's DataDisplay tables. v2.8.1: value/sale/year from the Property Account Summary page DataDisplay tables (`/API/DataDisplay/DataSources/GetData` keyed by table ModuleId off the page; tables ID'd by column name; MUST send `Accept: application/json` or the server 500s trying XML). v2.8.0: REBUILT Snohomish onto the portal QuickSearch JSON API after `snoco.org/proptax` was retired; `get_session`/`scrape_property` keep their signatures; results filtered to input street + city-disambiguated.** v2.7.4: KC direction-in-name street fallback. v2.7.3: KC condo master-parcel handling. v2.7.2: address fixes. | `python snoco_scraper_v2_8_3.py Seller_Leads.txt` |
