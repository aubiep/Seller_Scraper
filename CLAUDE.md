...
|---|---|---|
| `snoco_scraper_v2_8_0.py` | Lead enrichment. **King County LIVE** (ArcGIS + Dashboard). **Snohomish LIVE again** via the new Public Access portal. Writes to propintel.db. **v2.8.0: REBUILT Snohomish. The county retired `snoco.org/proptax` (every lookup 404'd); data now comes from the hosted portal's QuickSearch JSON API (`/DesktopModules/QuickSearch/API/Module/GetData?keywords=<house# + street name>&page=1`, with DNN headers ModuleId/TabId/RequestVerificationToken pulled from the search page). `get_session`/`scrape_property` keep their signatures (callers unchanged). Search result gives parcel+owner+situs = ownership verification; rich detail (value/beds/baths/year) is NOT in the public API and is left blank. Keywords drop street types/directions; results filtered to the input street + city-disambiguated, ambiguous skipped.** v2.7.4: KC relaxed fallback for direction-in-name streets (EAST->E). v2.7.3: KC condo master-parcel handling (walk unit parcels, match by surname). v2.7.2: address-clean / detect_county / street-filter fixes. | `python snoco_scraper_v2_8_0.py Seller_Leads.txt` |
