...
|---|---|---|
| `snoco_scraper_v2_7_4.py` | Lead enrichment. **King County LIVE** (ArcGIS + Dashboard, no bulk files) works. **Snohomish is currently DOWN: the county retired `snoco.org/proptax`; that path 404s and needs a rewrite to the new `wa-snohomish.publicaccessnow.com` portal.** Writes to propintel.db. **v2.7.4: KC relaxed address fallback recovers direction-in-name streets (kc_normalize abbreviates EAST->E, but 'East Ames Lake Dr' is stored '...EAST AMES LAKE DR...'); fallback keys on house# + core name words, accepts only an unambiguous single match.** v2.7.3: KC condo/townhome leads no longer land UNENRICHED (address resolves to the condo COMPLEX MASTER parcel MINOR 0000, common area; the scraper now walks per-unit parcels MAJOR-00X0 and matches by surname; absent/ambiguous match left unenriched). v2.7.2: address-clean / detect_county / street-filter fixes. | `python snoco_scraper_v2_7_4.py Seller_Leads.txt` |
